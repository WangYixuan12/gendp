...
from gild.policy.bc_unet_hybrid_image_policy import BCUnetHybridImagePolicy
from gild.dataset.base_dataset import BaseImageDataset
from gild.env_runner.base_image_runner import BaseImageRunner
from gild.common.checkpoint_util import TopKCheckpointManager
from gild.common.json_logger import JsonLogger
from gild.common.pytorch_util import dict_apply, optimizer_to
from gild.common.time_utils import get_time_now
from gild.model.diffusion.ema_model import EMAModel
from gild.model.common.lr_scheduler import get_scheduler

OmegaConf.register_new_resolver("eval", eval, replace=True)

class TrainBCUnetHybridWorkspace(BaseWorkspace):
    ...
    def __init__(self, cfg: OmegaConf, output_dir=None):
        ...
        # configure model
        self.model: BCUnetHybridImagePolicy = hydra.utils.instantiate(cfg.policy)

        self.ema_model: BCUnetHybridImagePolicy = None
        ...
        with JsonLogger(log_path) as json_logger:
            for local_epoch_idx in range(cfg.training.num_epochs):
                ...
                if (self.epoch % cfg.training.checkpoint_every) == 0: # and self.epoch > 0:
                    ...
                    time.sleep(1)
                    self.save_checkpoint(tag=f'epoch={self.epoch:03d}')
                    time.sleep(1)
...
def main(cfg):
    workspace = TrainBCUnetHybridWorkspace(cfg)
    workspace.run()
