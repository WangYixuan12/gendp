...
# %%
pointnet = PointNet()
pointnet.load_state_dict(torch.load('/home/yixuan/bdai/general_dp/general_dp/tests/save_8.pth'))
# pointnet.eval()

activations_and_grads = ActivationsAndGradients(pointnet, [pointnet.transform.conv3], None)
...
for batch_i, data in enumerate(valid_loader):
    ...
    # inputs = inputs + torch.randn_like(inputs) * 0.005
    inputs.requires_grad = True
    # outputs, m3x3, m64x64 = pointnet(inputs.transpose(1,2))
    outputs, m3x3, m64x64 = activations_and_grads(inputs.transpose(1,2))
    pointnet.zero_grad()
    ...
    cams = [np.mean(g, axis=-1) for g in grads_list] # [B, 1024]
    act_map = cams[0][..., None] * activations_list[0] # [B, 1024, N]
    act_map = act_map.sum(1) # [B, N]
    act_map = np.maximum(act_map, 0) # [B, N]
    act_map = act_map[:, None] # [B, 1, N]
    
    # get perpoint features
    norm_max = act_map[0].max()
    norm_min = act_map[0].min()
    ...
    for i in range(act_map.shape[1]):
        ...
        # feat_per_pt_curr_cls = act_map[i]
        # feat_per_pt_curr_cls = feat_per_pt_curr_cls.sum(1) # B, N
        feat_per_pt_curr_cls = act_map[0, i] # N

        # visualize
        raw_pcd = inputs[0].detach().cpu().numpy() # N, 3
        from d3fields.utils.draw_utils import np2o3d
        from matplotlib import cm
        cmap = cm.get_cmap('viridis')
        feat_per_pt_curr_cls_norm = (feat_per_pt_curr_cls - norm_min) / (norm_max - norm_min + 1e-8)
